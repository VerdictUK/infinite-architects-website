 ...
 * 
 * CORRECTED VERSION: January 16, 2026
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * VERIFIED FACTS:
 ...
 * - Gap: 5 DAYS
 * - Prediction: "practical quantum computing within approximately five years"
 * - BBC Confirmation: "Within the next five years" — Hartmut Neven, Google Quantum AI
 * - HRIH: Hyperspace Recursive Intelligence Hypothesis — a closed causal loop 
 *         where future superintelligence establishes conditions for its own emergence
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
(function() {
  ...
  const CONFIG = {
    breakpoint: 1023,
    buyBarScrollThreshold: 0.25, // 25% of hero height
    selectors: {
  ...
  const state = {
    isMobile: false,
    buyBarVisible: false,
    buyBarShownOnce: false, // Once shown, stays shown
  ...
  function loadHeavyAsset(element) {
    ...
    switch (assetType) {
      case 'equation-particles':
        // Initialize simplified particle animation
        initEquationParticles(element);
        break;
      case 'timeline-svg':
        // Placeholder for timeline loading
        element.innerHTML = '<div style="text-align: center; padding: 2rem; color: var(--accordion-gold);">Timeline Loaded</div>';
        break;
      default:
  ...
  function initEquationParticles(container) {
    // Simplified particle animation for equation section
    ...
    // Stop animation when accordion closes
    const accordion = container.closest('.accordion-item');
    if (accordion) {
      accordion.addEventListener('toggle', (e) => {
        if (!e.target.open && animationId) {
  ...
  function handleAccordionToggle(event) {
    ...
    if (accordion.open) {
      ...
      // Lazy load content
      content?.querySelectorAll(CONFIG.selectors.lazyVideo).forEach(loadVideo);
      content?.querySelectorAll(CONFIG.selectors.heavyAsset).forEach(loadHeavyAsset);
  ...
  function initBuyBar() {
    ...
    function updateBuyBar() {
      ...
      // Once shown, keep it shown (never hide again)
  ...
  function handleViewportChange() {
    ...
    if (wasMobile !== state.isMobile) {
      console.log('[Viewport]', state.isMobile ? 'Mobile' : 'Desktop');
      
      // Reset buy bar state when switching to desktop
      if (!state.isMobile) {
